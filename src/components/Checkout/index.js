    ...
    >
      ...
      <Modal.Body>
        <form onSubmit={e => { e.preventDefault(); props.onAccepted(); }} style={{ padding: '15px 80px 17px' }}>
          ...
          <input onChange={e => props.setInfo(prevState => ({ ...prevState, address: e.target.value }))} type='text' name='address' className={`mb-4 modal-input`} placeholder='Enter your Address...' required />
          <p className='mb-2 model-lable'><b>Phone</b></p>
          <input onChange={e => props.setInfo(prevState => ({ ...prevState, phone: e.target.value }))} type='text' name='phone' className={`mb-4 modal-input`} placeholder='Enter your Phone number...' required />
